...
public class StepSuggestionControllerTest {
    ...
    public void getStepsSuggestionsPageByType_StepsSuggestion_StatusOk() throws Exception {
        ...
        mockMvc.perform(
            get("/stepSuggestions?stepType=" + STEP_TYPE + "&page=" + PAGE_NUMBER + "&size="
                + PAGE_SIZE))
            .andExpect(status().isOk());

        verify(stepSuggestionService)
            .getStepsSuggestions(eq(STEP_TYPE), eq(PAGE_NUMBER), eq(PAGE_SIZE));
    }
    ...
    public void add_StepSuggestionWithNullContent_StatusBadRequest()
        ...
        verify(stepSuggestionService, times(0))
            .addStepSuggestion(any(StepSuggestionCreateDTO.class));
    }
